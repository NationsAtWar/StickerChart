...
public class StickerChartDatabaseFile extends StickerChartDataSource {
	...
	public boolean reloadDatabase() {
		...
		if (storageFile == null) {
			...
			try { storageFile = new File(plugin.getDataFolder(), "database.yml"); }
			catch(Exception e) {
	...
	public String getDisplayName(String playerName) {
		...
		return this.getDatabase().getString(playerName + ".playername");
	}

	@Override
	public boolean setAmount(String playerName, int amount) {
		
		getDatabase().set(playerName + ".amount", amount);
		
		if (getAmount(playerName) != amount)
			return false;
